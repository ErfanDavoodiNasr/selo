(function () {
  ...
  async function selectSearchResult(index) {
    const item = state.search.results[index];
    if (!item) return;
    try {
      if (item.chat_type === 'group') {
        const groupKey = item.public_handle || item.id;
        const joinRes = await apiFetch(API.group(groupKey) + '/join', { method: 'POST', body: {} });
        if (!joinRes.data.ok) {
          throw new Error(joinRes.data.error || 'خطا در عضویت گروه');
        }
        await loadConversations();
        const conv = state.conversations.find(c => c.id === joinRes.data.data.group_id && c.chat_type === 'group');
        if (conv) {
          await selectConversation(conv);
        }
      ...
      const convRes = await apiFetch(API.conversations, { method: 'POST', body: { user_id: item.id } });
      if (!convRes.data.ok) {
  ...
  function renderSearchResults() {
    ...
    state.search.results.forEach((result, index) => {
      const type = result.chat_type === 'group' ? 'group' : 'direct';
      if (type !== lastType) {
        const groupLabel = document.createElement('div');
        groupLabel.className = 'search-group-label';
        groupLabel.textContent = type === 'group' ? 'گروه‌ها' : 'کاربران';
      ...
      item.id = `search-result-${type}-${result.id}-${index}`;
      item.dataset.index = String(index);
      item.setAttribute('role', 'option');
      item.setAttribute('aria-selected', 'false');
      const fullName = type === 'group'
        ? (result.title || result.public_handle || 'گروه')
        : (result.full_name || result.username || 'کاربر');
      const username = type === 'group'
      ...
      if (type === 'group') {
        const icon = document.createElement('span');
        icon.className = 'material-symbols-rounded search-kind-icon';
        icon.textContent = result.is_member ? 'forum' : 'group_add';
  ...
  async function updateSearchResults(query) {
    ...
    try {
      const [usersRes, groupsRes] = await Promise.all([
        apiFetch(API.usersSearch + '?query=' + encodeURIComponent(query)),
        apiFetch(API.groupsSearch + '?query=' + encodeURIComponent(query))
      ]);
      if (requestId !== state.search.requestSeq) {
        return;
      }
      if (!usersRes.data.ok || !groupsRes.data.ok) {
        closeSearchResults({ invalidateRequest: false });
        return;
      }
      state.search.query = query;
      const users = Array.isArray(usersRes.data.data) ? usersRes.data.data.map(user => ({ ...user, chat_type: 'direct' })) : [];
      const groups = Array.isArray(groupsRes.data.data) ? groupsRes.data.data : [];
      state.search.results = [...groups, ...users];
      state.search.activeIndex = -1;
      renderSearchResults();
    } catch (err) {
      if (requestId !== state.search.requestSeq) {
